...
ag = function()
  ...
  if path ~= '' then
    vis:command(':e ' .. string.match(path, "(.-):"))
  end
end
...
vis.events.win_open = function(win)
  ...
  vis:command('map! insert <Backspace> <Escape>"_Xi')

  vis:command('map! visual <C-d> <S-PageDown>')
  vis:command('map! visual-line <C-d> <S-PageDown>')
  vis:command('map! visual <C-u> <S-PageUp>')
  vis:command('map! visual-line <C-u> <S-PageUp>')

  vis:map(vis.MODE_NORMAL, '<F3>', fzf)
  vis:map(vis.MODE_NORMAL, '<F7>', ag)
  vis:map(vis.MODE_NORMAL, '<F9>', vifm)
end
